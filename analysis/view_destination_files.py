...
# Set up logging
logging.basicConfig(filename='../reports/view_structure_source.log', level=logging.INFO, 
                    format='%(asctime)s,%(message)s', 
                    datefmt='%Y-%m-%d %H:%M:%S')

def log_item(item_type, path):
    """
    Log an item to the log file.
    """
    logging.info(f"{item_type},{path}")
...
def view_folder_contents(service, folder_id, current_path=""):
    ...
    for item in items:
        ...
        if is_folder(item):
            log_item("folder", item_path)
            view_folder_contents(service, item['id'], item_path)
        else:
            log_item("file", item_path)
